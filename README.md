...
| `--http-client-target`   | Optionally select the target the client that you want to be generated. Defaults to OkHttp                                                                                  |
|                          | CHOOSE ONE OF:                                                                                                                                                             |
|                          | `OK_HTTP` - Generate OkHttp Client.                                                                                                                                        |
|                          | `OPEN_FEIGN` - Generate OpenFeign interface.                                                                                                                               |
| `--http-client-opts`     | Select the options for the http client code that you want to be generated.                                                                                                 |
|                          | CHOOSE ANY OF:                                                                                                                                                             |
|                          | `RESILIENCE4J` - Only for OkHttp client target: Generates a fault tolerance service for the client using the following library "io.github.resilience4j:resilience4j-all:+" |
|                          | `SUSPEND_MODIFIER` - Only for OpenFeign client target: This option adds the suspend modifier to the generated methods in the interface                                     |
